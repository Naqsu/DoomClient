...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        if (this.itemToRender != null)
        {
            ...
            {
                // USTAWIENIA POZYCJI (X, Y, Z z modułu)
                float tx = 0, ty = 0, tz = 0;
                String mode = "1.7";

                if (anim != null && anim.isToggled()) {
                    tx = (float) anim.x.getValue();
                    ty = (float) anim.y.getValue();
                    tz = (float) anim.z.getValue();
                    mode = anim.mode.getMode();
                }

                switch (mode) {
                    case "1.7":
                        this.transformFirstPersonItem(f, swingProgress);
                        this.doBlockTransformations(); // Vanilla blokada
                    ...
                    case "Sigma":
                        ...
                        GlStateManager.translate(tx, ty, tz);
                        // Sigma manual transform (zamiast doBlockTransformations)
                        float sigmaVar = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float)Math.PI);
                        GlStateManager.translate(-0.05F, 0.2F, 0.2F);
                        GlStateManager.rotate(-sigmaVar * 70.0F / 2.0F, -8.0F, -0.0F, 9.0F);
                        GlStateManager.rotate(-sigmaVar * 70.0F, 1.0F, -0.4F, -0.0F);
                        this.doBlockTransformations(); // Sigma akurat bazuje na vanilli, więc tu zostawiamy
                        break;

                    case "Exhibition":
                        // Exhibition NIE używa doBlockTransformations, robimy to ręcznie
                        this.transformFirstPersonItem(f, 0.0F);
                        GlStateManager.translate(tx, ty, tz);
                        ...
                        float exhiVar = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float) Math.PI);
                        GlStateManager.rotate(exhiVar * -35.0F, 1.0F, -0.5F, 2.0F);
                        // USUNIĘTO this.doBlockTransformations() - to powodowało znikanie!
                        break;
                    ...
                    case "Slide":
                        ...
                        GlStateManager.translate(tx, ty, tz);
                        float slideVar = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float)Math.PI);
                        GlStateManager.translate(-0.4F, 0.2F, 0.0F);
                        GlStateManager.rotate(-slideVar * 35.0F, -8.0F, -0.0F, 9.0F);
                        GlStateManager.rotate(-slideVar * 10.0F, 1.0F, -0.4F, -0.5F);
                        break;
                    ...
                    case "Swang":
                        ...
                        this.doBlockTransformations();
                        GlStateManager.translate(tx, ty, tz);
                        float swangVar = MathHelper.sin(MathHelper.sqrt_float(swingProgress) * (float)Math.PI);
                        GlStateManager.rotate(swangVar * 30.0F / 2.0F, -8.0F, -0.0F, 9.0F);
                        GlStateManager.rotate(swangVar * 40.0F, 1.0F, -0.4F, -0.0F);
                        break;

                    default: // Fallback (np. "System")
                        this.transformFirstPersonItem(f, 0.0F);
                        this.doBlockTransformations();
                        GlStateManager.translate(tx, ty, tz);
                        break;
            ...
            {
                // To jest sekcja odpowiedzialna za widoczność miecza, gdy NIE blokujesz
                // Jeśli miecz znikał też bez aury, to wina leżała w złym if/else powyżej.
                ...
                if (abstractclientplayer.getItemInUseCount() > 0) {
                    ...
                    // Zwykłe trzymanie w ręce (IDLE / SWING)
